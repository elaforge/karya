...
test_normalizeSpeedGroups = do
    let f = map (fmap extract) . normalizeSpeed adiAksharas . S.flatten
        n = Note (1 :: S.Matra)
        extract = pretty . snd
    let t0 = tempo 0 4
        t1 = tempo 1 4
...
normalizeSpeed talaAksharas notes =
    S.normalizeSpeed (S.maxSpeed notes) talaAksharas notes
