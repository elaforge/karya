...
OSStatus core_midi_initialize(const char *name, ReadCallback read_cb,
...
OSStatus core_midi_connect_read_device(DeviceId rdev, void *p);
OSStatus core_midi_disconnect_read_device(DeviceId dev);
OSStatus core_midi_write_message(DeviceId wdev, Timestamp timestamp, int len,
    const unsigned char *bytes);

// misc
OSStatus core_midi_abort();
