...
test_cmds_with_note = do
    ...
    let ckey = ctrl_key (Key.Char char)
    equal (run (with_key (ctrl_key Key.ControlL)) (f True ckey))
        (through ckey)

    let st = CmdTest.default_cmd_state
    equal (run st (f True (CmdTest.key_down char)))
        (through $ input (note_on 60 5 0 0))
    equal (run st (f True (CmdTest.key_up char)))
        (through $ input (CmdTest.note_off 60))

    let oct5 = st { Cmd.state_edit = (Cmd.state_edit st)
            { Cmd.state_kbd_entry_octave = 5 } }
    equal (run oct5 (f True (CmdTest.key_down char)))
        (through $ input (note_on 72 6 0 0))
    -- with kbd_entry off, keys don't get mapped
    equal (run st (f False (CmdTest.key_down char)))
        (through (CmdTest.key_down char))
