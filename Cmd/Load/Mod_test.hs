...
tempo :: ModT.Tempo
tempo = ModT.Tempo 33 6

inst :: ModT.Instrument
inst = ModT.Instrument (ScoreT.Instrument "inst") (Just 0.5)

initial_state :: Mod.State
initial_state = Mod.State
    { _tempo = ModT.Tempo 33 6
    , _instruments = IntMap.fromList [(0, inst)]
    }

test_convert_block = do
    let f = Mod.convert_block initial_state
    let empty = ModT.Line Nothing 0 []
    let tracks = map ModT.make_track
            [ zip [0..] [empty, empty]
            , zip [0..] [empty, empty]
            ]
    -- TODO I think this was supposed to do something?
    pprint (f (ModT.Block tracks 2))
...
test_convert_note_controls = do
    ...
    let up = ModT.VolumeSlide 1
    equal (e_controls $ f 0 [up] []) [("dyn", [(0, 1/8, "from=`0x`80 | u 1")])]
    -- volume + crescendo
    equal (e_controls $ f 0 [ModT.Volume 0, up] [])
        [("dyn", [(0, 1/8, "from=`0x`00 | u 1")])]
    -- grouping
    equal (e_controls $ f 0 [up] [(1, cmd up), (2, cmd (ModT.VolumeSlide 2))])
        [ ( "dyn"
