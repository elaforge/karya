...
save_state_as fn = Save.save_state_as =<< liftIO (Save.expand_filename fn)

-- | Like 'save_state', but don't change 'Cmd.state_save_file'.
write_state :: FilePath -> Cmd.CmdL ()
write_state fn = Save.write_current_state =<< liftIO (Save.expand_filename fn)
...
save_git_as fn = Save.save_git_as =<< liftIO (Save.expand_filename fn)
