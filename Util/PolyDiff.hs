-- | This is a copy-paste of Data.Algorithm.Diff from the Diff package, which
-- has a BSD3 license, below.  The reason is that I'd like to diff different
-- typed lists.  Diff can do that, just by generalizing the type signatures.
...
data Diff a b = First a | Second b | Both a b deriving (Show, Eq)
...
getGroupedDiff :: (Eq t) => [t] -> [t] -> [Diff [t] [t]]
...
getDiffBy :: (a -> b -> Bool) -> [a] -> [b] -> [Diff a b]
...
getGroupedDiffBy :: (a -> b -> Bool) -> [a] -> [b] -> [Diff [a] [b]]
