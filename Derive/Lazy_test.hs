...
test_two = do
    ...
    equal evaluated
        [ "b1 b1.t01 0-1 note at: 0s"
        , "b1 b1.t02 0-1 note at: 0s"
        , "b1 b1.t01 1-2 note at: 1s"
        , "b1 b1.t02 2-3 note at: 2s"
        , "b1 b1.t01 2-3 note at: 2s"
        ]
...
test_everything = do
    ...
    evaluated <- get_log log
    equal evaluated
        ["b1 b1.t02 0-1 note at: 0s", "sub sub.t01 0-1 note at: 2s"]
    where
...
test_track_signal = do
    ...
    (log, res) <- derive_block ustate
    let t1_derived = length . filter ("b1.t02" `List.isInfixOf`) <$> get_log log
