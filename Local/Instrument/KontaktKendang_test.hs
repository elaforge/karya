...
test_kendang = do
    let run extract inst notes = DeriveTest.extract extract $ derive
        ...
        k = "kkt/kendang"
    equal (run e_inst "kkt/kendang" ["PL", "P", "o"])
        ([(k, "+plak"), (k, "+pak"), (k, "+tut")], [])
    equal (run e_inst "kkt/kendang-pasang" ["PL", "k", "P", "t", "T", "u", "U"])
        ([("w", "+plak"), ("w", "+pak"), ("l", "+pak"),
            ("w", "+pang"), ("l", "+pang"), ("w", "+tut"), ("l", "+tut")], [])

    -- Soft attributes.
    let e_dyn e = (DeriveTest.e_attributes e, Score.initial_dynamic e)
    equal (run e_dyn "kkt/kendang" [".", "..", "-", "+"])
...
derive = DeriveTest.derive_tracks_with
    ...
    where aliases = [("w", "kkt/kendang"), ("l", "kkt/kendang")]
