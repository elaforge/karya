...
test_natural_harmonic = do
    ...
    equal (run ">vsl/violin" "+harmonic+nat" "4c")
        ([], ["Error: c4 unplayable on [ +g, +d, +a, +e ]"])
    equal (run ">vsl/violin" "+harmonic+nat" "3g") ([Key.c3], [])
    equal (run ">vsl/violin" "+harmonic+nat" "4g") ([Key.d3], [])
    equal (run ">vsl/violin" "+harmonic+nat" "6d") ([Key.gs3], [])
    equal (run ">vsl/violin" "+harmonic+nat+d" "6d") ([Key.f4], [])
